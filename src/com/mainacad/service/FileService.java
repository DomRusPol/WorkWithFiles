...
public class FileService {
	...
	public static void writeTextToFile(String text, String fileName, boolean append) {
		...
		try (FileWriter fileWriter = new FileWriter(FILES_DIR + SEP + fileName, append)) { // Вопрос про закрытие
...
//        try {  
//            fileWriter = new FileWriter(FILES_DIR + SEP + fileName, append);
//            fileWriter.write(text + "\n");
//            fileWriter.flush();
//        } catch (IOException e) {
//            e.printStackTrace();
...
//			}
	...
	// Используется при перемещении файла в новую директорию.
	...
	public static List<ConnectionInfo> readConectionsFromFile(String fileName) {
		...
		try (FileReader fileReader = new FileReader(FILES_DIR + SEP + fileName);
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				connectionInfo.setSessionId(Integer.valueOf(words[1]));
				connectionInfo.setConnectionTime(Long.valueOf(words[0]));
	...
	public static List<ConnectionInfo> readConectionsFromFileByPeriod(String fileName, long startTime, long endTime) {
		List<ConnectionInfo> connectionInfoList = new ArrayList<>();
		...
		try (FileReader fileReader = new FileReader(FILES_DIR + SEP + fileName);
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				connectionInfo.setConnectionTime(Long.valueOf(words[0]));
				connectionInfo.setIp(words[2]);
				if (connectionInfo.getConnectionTime() >= startTime && connectionInfo.getConnectionTime() <= endTime) {
	...
	public static void deleteExpiredData(String fileName, long expiredDate) { // new Date().getTime() - 1000*60*60*24*3
		List<ConnectionInfo> connectionInfoList = readConectionsFromFile(fileName);
		List<ConnectionInfo> filteredList = new ArrayList<ConnectionInfo>();
		for (ConnectionInfo connectionInfo : connectionInfoList) {
			if (connectionInfo.getConnectionTime() >= expiredDate) {
				filteredList.add(connectionInfo);
			}
		}

//		writeTextToFile("", fileName, false);
		boolean append = false;
		for (ConnectionInfo connectionInfo : filteredList) {
			writeTextToFile(connectionInfo.toString(), fileName, append);
			append = true;
		}
